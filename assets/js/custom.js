jQuery(document).ready(function($) {
  ...
  $(document).on('mouseenter','#main_menu li.dropdown', function (event) {
      $(this).find('.dropdown-menu').stop(true, true).delay(200).fadeIn();
  }).on('mouseleave','#main_menu li.dropdown',  function(){
    ...
    ruta = window.location.href.toString().split(window.location.host)[1];
    if( ruta == '/' || ruta == '' && window.location.href.indexOf('?') !== -1)
        $("#content_dock").load("/content/home.html");
    else{
        load_content_page();
    }
    ...
    //MENU
    // $(document).on('click', ".accordion_menu_mobile", function (event) {
    //     //$(".accordion_menu_mobile panel").style.maxHeight = null;//.removeClass("active");
...
{
    pagina = window.location.href.toString().split('?')[1];
    $("#content_dock").load("content/"+pagina+".html");

    $("#main_menu .dropdown").removeClass("active");
    var id = pagina.split('/')[0];
